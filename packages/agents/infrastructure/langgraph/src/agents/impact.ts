...
>() {
	...
	private static impactNode = (
      ...
      `)
			...
			const impactDimensions = yield* $(
				Effect.promise(() =>
					ImpactAgent.parseImpactDimensions(
						[...dimensions],
			...
			const summary = yield* $(
				Effect.promise(() =>
					ImpactAgent.generateImpactSummary(
						impactDimensions,
						overallImpact,
						model,
					),
			...
			if (includeRecommendations) {
				recommendations = yield* $(
					Effect.promise(() =>
						ImpactAgent.generateRecommendations(impactDimensions, model),
					),
				)
			}
	...
	/**
	 * Live implementation of the ImpactAgent
	 */
